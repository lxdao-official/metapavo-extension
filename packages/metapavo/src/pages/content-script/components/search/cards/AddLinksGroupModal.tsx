...
import { createUserTag } from 'extension-common/src/apis/links_api';
import { getLang } from 'extension-common/src/lang';
import { useEffect, useState } from 'react';
import toast from 'react-hot-toast';

export function AddLinksGroupModal() {
  ...
  async function submitUserCategory() {
    ...
    if (addCategoryTitle.length > 0) {
      try {
        ...
        setaddCategoryDesc('');
        toast.success('saved');
        globalEvent.emit('add_links_group_success');
      } catch (e) {
  ...
  useEffect(() => {
    globalEvent.on('add_links_group', () => {
      showModal();
    });
    return () => {};
  }, []);
