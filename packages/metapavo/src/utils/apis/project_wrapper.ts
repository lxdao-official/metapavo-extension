...
export const projectLinksWrapper = (project: IProjectV2) => {
  const links: Links = {
    ...
    opensea: project?.symbol ? `https://opensea.io/collection/${project.symbol}` : undefined,
    looksrare: project?.contractAddress
      ? `https://looksrare.org/collections/${project.contractAddress}`
      : undefined,
    x2y2: project?.symbol ? `https://x2y2.io/collection/${project.symbol}/items` : undefined,
    gem: project?.symbol ? `https://www.gem.xyz/collection/${project.symbol}` : undefined,
    ...
    nfteye: project.symbol ? `https://nfteye.io/collections/${project.symbol}` : undefined,
