...
export default function CoinNormalCard(props: { token: tokens }) {
  ...
  return (
    <TokenCardRoot>
      <div className="mp-success-hd">
        ...
        >
          {props.token.symbol} {props.token.name}
        </a>
      ...
      <div className="mp-success-bd">
        <div className="mp-success-bd-price">
          <span className="mp-success-price-item">
            {getLang('low24h')}:{' '}
            {props.token.low24h ? AutoDecimal(props.token.low24h) : '-'} U
          </span>
          <span className="mp-success-price-item">
            {getLang('high24h')}:{' '}
            {props.token.high24h ? AutoDecimal(props.token.high24h) : '-'} U
            {props.token.percentChange24h ? (
              <span
                style={{
                  color:
                    props.token.percentChange24h > 0 ? '#3EAF3F' : '#E14942',
                  paddingLeft: '5px',
                  fontWeight: 500,
                }}
              >
